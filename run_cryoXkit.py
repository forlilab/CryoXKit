...
if args[0].convert2mrc is not None:
    cxk = CryoXKit()
    cxk.Convert2MRC(args[0].convert2mrc)
    exit(0)
...
cxk = CryoXKit(args.map_file, args.map_receptor, args.rmsd_cutoff, args.gaussian_filter_sigma, args.noise_std_range, args.repeat_unit_cell, args.output_aligned_receptors)
grid_maps      = cxk.ReadGridMaps(args.grid_maps)
density        = cxk.ReadMapFiles()
if args.output_intermediate_densities is True:
    cxk.WriteDensity(density, "read_density")
mask           = cxk.CreateMask(density, args.mask_file)
masked_density = cxk.ApplyMask(density, mask)
if args.output_intermediate_densities is True and args.mask_file is not None:
    cxk.WriteDensity(masked_density, "masked_density")
modified       = cxk.ModifyDensity(masked_density, args.log_min, args.log_width, args.log_x0)
if args.output_intermediate_densities is True:
    cxk.WriteDensity(modified, "modified_density")
cxk.WriteGridMaps(modified, 1)
